...

{
    ...
    "category": "function",
    "text": "genpointings(wheelanglesfn, dir, timerange_s; \n             polaxis=Float64[1.0, 0.0, 0.0],\n             latitude_deg=0.0, ground=false)\ngenpointings(wheelanglesfn, dir, timerange_s, t_start;\n             polaxis=Float64[1.0, 0.0, 0.0],\n             latitude_deg=0.0, longitude_deg=0.0, height_m=0.0)\n\nGenerate a set of pointings for a STRIP detector. The parameter wheelanglesfn must be a function which takes as input a time in seconds and returns a 3-tuple containing the angles (in radians) of the three motors:\n\nThe boresight motor (rotation around the z axis, counterclockwise)\nThe altitude motor (rotation around the y axis, counterclockwise)\nThe ground motor (rotation around the z axis, clockwise: N→E→S→W)\n\nThe meaning of the parameters/keywords is the following:\n\ndir specifies the pointing direction of the mean (the boresight is [0, 0, 1]). It must be normalized.\ntimerange_s is an enumerable type that specifies at which times (in seconds) pointings must be computed.\nt_start is a DateTime which tells the exact UTC date and time of the  observation.\nlatitude_deg is the latitude of the location where the observation is made (in degrees, North is positive).\nground is a Boolean: if true, the function will return a 4-tuple containing the colatitude and longitude measured in Equatorial coordinates (columns 1 and 2) and in ground coordinates (columns 3 and 4). If false, only the Equatorial coordinates are computed.\npolaxis is the polarization axis; it must be normalized.\nprecession is a boolen parameter that allow to include the Earth\'s precession effect; default is true.\nnutation is a boolen parameter that allow to include the Earth\'s nutation effect; default is true.\naberration is a boolen parameter that allow to include the stellar aberration effect; default is true.\nrefract is a boolen parameter that allow to include the refraction correction ; default is true.\n\nReturn values\n\nIn the first form, the function returns a 2-tuple containing the sky directions (a N×2 array containing declination and right ascension, in Equatorial coordinates) and the polarization angle for each time step.\n\nIn the second form, the function returns a 2-tuple (4-tuple) containing the directions (a N×2 or Nx4 array containing the colatitude and the longitude) and the polarization angles at each time step.\n\nExamples\n\nHere is an example of the first kind:\n\ndir, psi = genpointings([0, 0, 1], 0:0.1:1) do time_s\n    # Boresight motor keeps a constant angle equal to 0°\n    # Altitude motor remains at 20° from the Zenith\n    # Ground motor spins at 1 RPM\n    return (0.0, deg2rad(20.0), timetorotang(time_s, 1))\nend\n\nAnd here is an example of the second kind. Note that, unlike the previous example, we use a lambda function instead of the do...end notation.\n\nimport Dates\n\ndirs, psi = genpointings(time_s -> (0, deg2rad(20),\n                                    timetorotang(time_s, 1)),\n                         [0, 0, 1],\n                         0:0.1:1,\n                         Dates.DateTime(2019, 01, 01, 0, 0, 0),\n                         latitude_deg=10.0,\n                         longitude_deg=20.0,\n                         height_m = 1000) do time_s\n\n\n\n\n\n"
},
...

{
    ...
    "category": "function",
    "text": "tod2map(pix_idx, tod, num_of_pixels, comm) -> binned_map\ntod2map(pix_idx, tod, num_of_pixels, data_properties, comm) -> binned_map\n\nThis function creates a binned map from the time-ordered data kept in the array tod, assuming that each sample is observing the pixel whose index is in pix_idx. The parameter num_of_pixels contains the number of pixels in the Healpix map, and it is used as an upper bound for the values in pix_idx. The parameter comm must be a MPI communicator, or missing if you are not using MPI. This is a MPI based function: each MPI process computes a map from its available data.  All partial maps are then combined together with MPI.allreduce. The function returns an array containing the binned map.\n\nIf Array of structures data_properties_structis passed to the function, the output binned map will be a weighted binned map. Each sample will be weighted according to the inverse white noise variance σ^2 of the corrisponding polarimeter. In this way, the less noisy polarimeters will count more in the estimation of the map.\n\nRequirements\n\nThe length of the arrays pix_idx and tod must be the same\n\n\n\n\n\n"
},
