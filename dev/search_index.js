...

{
    ...
    "category": "function",
    "text": "tod2map(pix_idx, tod, num_of_pixels, comm) -> binned_map\n\nThis function creates a binned map from the time-ordered data kept in the array tod, assuming that each sample is observing the pixel whose index is in pix_idx. The parameter num_of_pixels contains the number of pixels in the Healpix map, and it is used as an upper bound for the values in pix_idx. The parameter comm must be a MPI communicator, or missing if you are not using MPI.\n\nThis is a MPI based function: each MPI process computes a map from its available data.  All partial maps are then combined together with MPI.allreduce.\n\nThe function returns an array containing the binned map.\n\nRequirements\n\nThe length of the arrays pix_idx and tod must be the same;\n\n\n\n\n\n"
},

{
    ...
    "category": "function",
    "text": "baseline2map_mpi(pix_idx, baselines, baseline_lengths, num_of_pixels, comm; unseen=NaN)\n\nThis function creates a binned map from the sequence of baselines in baselines. Each baseline covers a number of samples equal to the corresponding element in baseline_lengths. The function assumes that each sample is observing the pixel whose index is in pix_idx. The parameter num_of_pixels contains the number of pixels in the Healpix map, and it is used as an upper bound for the values in pix_idx. The parameter comm must be a MPI communicator, or missing if you are not using MPI.\n\nThis is a MPI based function: each MPI process computes a map from its available data.  All partial maps are then combined together with MPI.allreduce.\n\nThe function returns an array containing the binned map.\n\nRequirements\n\nThe length of baselines and baseline_lengths must be the same;\nThe value sum(baseline_lengths) must be the same as the length of pix_idx.\n\n\n\n\n\n"
},
